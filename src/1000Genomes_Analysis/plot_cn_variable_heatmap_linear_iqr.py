...
fig.suptitle(
    "Copy-Number-Variable VNTRs — Heatmap + Linear Regression (IQR zoom)\n(vamosExpanded v3.0 GRCh38 oriMotifs)",
    fontsize=13,
)
plt.tight_layout()

out_path = os.path.join(out_dir, "cn_variable_heatmap_linear_iqr.png")
plt.savefig(out_path, dpi=150, bbox_inches="tight")
plt.close()
print(f"Heatmap (linear, IQR zoom) saved to: {out_path}")
